...
/**
 * 翻译引擎：支持 Gemini (免费) / OpenAI (付费) / Rapunzel (自定义)
 */
...
6. Output ONLY the translation, no explanation${termPrompt}${tmPrompt}`;
  ...
  try {
    ...
    } else if (engine === 'rapunzel') {
      translated = await rapunzelTranslate(segment.source, terms);
    } else {
...
async function rapunzelTranslate(source, terms) {
  const apiUrl = process.env.RAPUNZEL_API_URL;
  const apiKey = process.env.RAPUNZEL_API_KEY;
  if (!apiUrl || !apiKey) throw new Error('Rapunzel API not configured');
